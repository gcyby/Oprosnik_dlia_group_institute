...
def time_seckond(format_time):
    # ункция перевода времени на дату и колличество секунд
    ...
    time_vote_second = int(hour_minute_seconds[0]) * 3600 + int(hour_minute_seconds[1]) * 60 + int(
        hour_minute_seconds[2]) + mouth_seconds
...
def id_user_in_table(file_name: str, table_name: str, litera_stolbza: str):
    # Функция получение всех id из листа в эксэле
    id_admin = []
    data_base = load_workbook(f'{file_name}')
    table = data_base[f'{table_name}']
    cnt = 1
    while table[f'{litera_stolbza}{cnt}'].value is not None:
        id_admin.append(table[f'{litera_stolbza}{cnt}'].value)
        cnt += 1
...
def sozdaem_opros(danie_oprosa, id_kogoto):
    ...
    if colichestvo_voprosov == len(danie_oprosa):
        ...
        new_sheet["A1"], new_sheet["B1"], new_sheet["C1"], new_sheet[
            "D1"] = 'id', 'имя', 'Ответ', 'Время ответа'
...
if __name__ == '__main__':
    ...
    @bot.message_handler(commands=['dobavit_beseda'])
    def dobavit(message):
        if message.from_user.id in Admin_id:
            id_group_table = id_user_in_table('база старст_групп.xlsx', 'Group', 'B')
            name_base = 'база старст_групп.xlsx'
            id_starosti = message.from_user.id
            id_group_beseda = message.chat.id
            number_group = user_name(id_starosti, name_base, 'Admin_List', 'C')
            if number_group not in id_group_table:
                datebase = load_workbook(name_base)
                sheet = datebase['Group']
                cnt = 1
                while sheet[f'A{cnt}'].value is not None:
                    cnt += 1
                sheet[f'A{cnt}'], sheet[f'B{cnt}'], sheet[f'C{cnt}'] = id_starosti, number_group, id_group_beseda
                datebase.save(name_base)
                bot.send_message(message.chat.id, 'Вы добавили ид беседы в бота')
            else:
                bot.send_message(message.chat.id, 'Беседа этой группы уже зарегестрирована')
    ...
    def registration_starost(message):
        ...
        admin_lst = id_user_in_table('база старст_групп.xlsx', 'Admin_List', 'A')
        if message.from_user.id not in admin_lst:
            cnt = 1
            while sheet[f'A{cnt}'].value is not None:
                cnt += 1
            sheet[f'A{cnt}'].value, sheet[f'B{cnt}'].value, sheet[
                f'C{cnt}'].value = message.from_user.id, name, number_group
    ...
    def registration_student(message):
        ...
        dannie = message.text.split(', ')
        masiv_id_group = id_user_in_table(f'{dannie[1]} база посещений.xlsx', 'id_Group', 'A')
        if message.from_user.id not in masiv_id_group:
            ...
            while sheet[f'A{cnt}'].value is not None:
                cnt += 1
            sheet[f'A{cnt}'], sheet[f'B{cnt}'], sheet[
                f'C{cnt}'] = message.from_user.id, message.from_user.username, int(dannie[0])
    ...
    def sozdanie_fila_group(message):
        ...
        if cnt == 0:
            ...
            sheet['A2'].value, sheet['B2'].value, sheet['C2'].value = message.from_user.id, message.from_user.username, \
                dannie[0]
    ...
    def pull_make(message):
        ...
        if len(qestion) > 3 and message.from_user.id in Admin_id and qestion[1] in '123456780':
            ...
            bot.send_message(message.chat.id,
                             'Неправильный ввод данных Пример: "ВышМат-название, 2-колличество вопросов, был, не был"')
    ...
    def callback_message(callback):
        ...
        if 'vote' in callback.data:
            ...
            masiv_id = id_user_in_table(name_data_base, callback.message.json.get('text'), 'A')
            if (id_user not in masiv_id) or (
                    id_user in masiv_id and sravnim_vremya(masiv_id, data_base, callback, id_user) == True):
                ...
                for i in range(len(callback.message.json.get('reply_markup').get('inline_keyboard'))):
                    if callback.message.json.get('reply_markup').get('inline_keyboard')[i][0].get(
                        ...
                        sheet[f'A{namber_str}'], sheet[f'B{namber_str}'], sheet[f'C{namber_str}'], sheet[
                            ...
                            callback.message.json.get('reply_markup').get('inline_keyboard')[i][0].get(
                                'text'), time_vote
                        data_base.save(f'{number_group} база посещений.xlsx')
                        if id_user not in masiv_id:
                            bot.answer_callback_query(callback.id, "Спасибо за голос")
                        else:
                            bot.answer_callback_query(callback.id, "Ваш голос был изменён")
                else:
                    bot.answer_callback_query(callback.id, "Вы уже голосовали")

        elif callback.data == 'sozdate_opros':
            # Создаёт сообщение прокладку для перехода в функцию создания опроса.
            bot.send_message(callback.message.chat.id,
                             'Назави опрос, колличество ответов и сами ответы через запятую c пробелом')
